...
BranchMe is a TypeScript Pi extension package for Git repository initialization, current-repository branch, verified branch-integration and retirement, linked-worktree, push, and GitHub pull request workflows.
...
1. `src/extension.ts` stays small and registers the command, nineteen tools, and one `before_agent_start` context hook.
...
- Automatic and explicit context include branch/upstream/ahead-behind state, working-tree counts, up to 20 unstaged/untracked entries, related open PR state, and up to 5 recent commits. Explicit `branch_status` may additionally verify one strict local or remote-tracking source/target ancestry query; automatic context never does. Metadata values default to 512 characters and rendered context to 4,000 characters.
...
├── workflow.test.mjs     # discovery, verified remote tracking, base merges and conflicts
├── pr-workflow.test.mjs  # PR lifecycle validation, idempotence, races and schema

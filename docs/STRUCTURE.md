...
1. `src/extension.ts` stays small and registers the command, five tools, and one `before_agent_start` context hook.
...
- Slash commands are informational; tools perform branch, push, and PR actions. There is no context command or sixth tool.

...
test("packaged project brief describes current worktree behavior", async () => {
  ...
  assert.match(projectBrief, /nineteen strict agent-callable tools/i);
  for (const toolName of ["init_repository", "list_worktrees", "create_worktree", "remove_worktree", "retire_branch"]) {
  ...
  assert.match(projectBrief, /ignored entries block removal by default.*deleteIgnored: true/is);
...
test("source tree registers BranchMe behavior and no template leftovers", async () => {
  ...
  assert.match(constants, /init_repository/);
...
test("branch retirement contracts are bounded and registered atomically", async () => {
  ...
  assert.equal(BRANCHME_TOOL_NAMES.length, 19);
...
test("public documentation describes implemented behavior", async () => {
  ...
  assert.match(activeDocumentation, /nineteen (?:agent-callable )?tools/iu);
  ...
  assert.match(readme, new RegExp(INIT_REPOSITORY_TOOL_NAME, "u"));
